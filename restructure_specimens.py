...
# 1. Generate new HTML for curated-specimens.html
...
admin = admin.replace("loadCollection('curated-specimens')", "loadPageContent('specimens')")
admin = admin.replace("currentSelection === 'curated-specimens'", "currentSelection === 'specimens'")
...
with open('database_seeder.py', 'r') as f:
    seeder = f.read()
...
if "specimens/hero/title" not in seeder:
    parts = seeder.split('\\n}\\n', 1)
    seeder = parts[0] + seeds + '\\n}\\n' + parts[1]
    with open('database_seeder.py', 'w') as f:
        f.write(seeder)
